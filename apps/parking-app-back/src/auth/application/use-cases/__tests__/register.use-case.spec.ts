import { RegisterUseCase } from '@/auth/application/use-cases';
import { CreateUserUseCase } from '@/users/application/use-cases';
import { GenerateTokenUseCase } from '@/auth/application/use-cases';
import { User } from '@/users/domain/entities/user.entity';
import { LoginDto, JwtPayload } from '@/auth/application/dtos';
...
describe('RegisterUseCase', () => {
  ...
  it('should throw if user creation fails', async () => {
    ...
    await expect(registerUseCase.execute(dto)).rejects.toThrow('Creation failed');
  });
